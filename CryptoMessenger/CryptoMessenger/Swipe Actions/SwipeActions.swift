                    ...
                    .onChanged { value in
                        ...
                        withAnimation {
                            if visibleButton == .left && value.translation.width < -20 { ///user dismisses left buttons
								print("gesture user dismisses left buttons")
                                reset()
                            } else if visibleButton == .right && value.translation.width > 20 { ///user dismisses right buttons
								print("gesture user dismisses right buttons")
                                reset()
                            } else if offset > 20 || offset < -20 { ///scroller more then 50% show button
								print("gesture scroller more then 50% show button")
								action?()
								// Пока отключил, чтобы ячейка возвращалась обратно
//                                if offset > 0 {
//                                    visibleButton = .left
