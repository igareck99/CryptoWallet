...
class MXSpaceGraphData: NSObject, NSCoding {
	...
	required init(coder aDecoder: NSCoder) {
		self.spaceRoomIds = aDecoder.decodeObject(forKey: Constants.spaceRoomIdsKey) as? [String] ?? []
		self.parentIdsPerRoomId = aDecoder.decodeObject(forKey: Constants.parentIdsPerRoomIdKey) as? [String : Set<String>] ?? [:]
		self.ancestorsPerRoomId = aDecoder.decodeObject(forKey: Constants.ancestorsPerRoomIdKey) as? [String : Set<String>] ?? [:]
		self.descendantsPerRoomId = aDecoder.decodeObject(forKey: Constants.descendantsPerRoomIdKey) as? [String : Set<String>] ?? [:]
		self.rootSpaceIds = aDecoder.decodeObject(forKey: Constants.rootSpaceIdsKey) as? [String] ?? []
		self.orphanedRoomIds = aDecoder.decodeObject(forKey: Constants.orphanedRoomIdsKey) as? Set<String> ?? Set()
		self.orphanedDirectRoomIds = aDecoder.decodeObject(forKey: Constants.orphanedDirectRoomIdsKey) as? Set<String> ?? Set()
	}

	func encode(with aCoder: NSCoder) {
		aCoder.encode(self.spaceRoomIds, forKey: Constants.spaceRoomIdsKey)
		aCoder.encode(self.parentIdsPerRoomId, forKey: Constants.parentIdsPerRoomIdKey)
		aCoder.encode(self.ancestorsPerRoomId, forKey: Constants.ancestorsPerRoomIdKey)
		aCoder.encode(self.descendantsPerRoomId, forKey: Constants.descendantsPerRoomIdKey)
		aCoder.encode(self.rootSpaceIds, forKey: Constants.rootSpaceIdsKey)
		aCoder.encode(self.orphanedRoomIds, forKey: Constants.orphanedRoomIdsKey)
		aCoder.encode(self.orphanedDirectRoomIds, forKey: Constants.orphanedDirectRoomIdsKey)
	}
